...
void KalmanFilter::handleLidarMeasurement(LidarMeasurement meas, const BeaconMap& map)
{
    if (isInitialised())
    {
        ...
        {           
            ...
            double delX = map_beacon.x - pos_x;
            double delY = map_beacon.y - pos_y;
            double lidarDist = sqrt(delX*delX + delY*delY);
            double lidarAngle = wrapAngle(atan2(delY,delX) - psi);   

            VectorXd z_hat = Vector2d::Zero(); // Predicted Measurement
            VectorXd z = Vector2d::Zero();     // True Measurement
            VectorXd y = Vector2d::Zero();     // Innovation
 
            z_hat << lidarDist,lidarAngle;
            z << meas.range,meas.theta;
            y = z - z_hat;

            MatrixXd H = MatrixXd(2,4);        // Jacobian for linearizing measurement model
            MatrixXd R = Matrix2d::Zero();     // Covariance Matrix of measure
            H << (-delX/lidarDist),(-delY/lidarDist),0,0,(delY/(lidarDist*lidarDist)),(-delX/(lidarDist*lidarDist)),-1,0;
